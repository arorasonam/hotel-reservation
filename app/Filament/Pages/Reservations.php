...
class Reservations extends Page
{
    ...
    {
        try {
            $roomBlock = ReservationRoom::find($roomStayId);
            if (! $roomBlock) {
                return ['success' => false, 'message' => 'Room block not found'];
            }

            // 1. Update the individual room status
            $roomBlock->update([
                'status' => $status,
                'checked_in_at' => $status === 'checked_in' ? now() : $roomBlock->checked_in_at,
            ...
            if ($roomBlock->room_number) {
                ...
                HotelRoom::where('room_number', $roomBlock->room_number)->update(['status' => $physicalStatus]);
            }
